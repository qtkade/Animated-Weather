...
// here will be our QtObject[channelObj] from QML side
//html variables <div> for change the text labels
...
function createChannel()
{
    new QWebChannel(qt.webChannelTransport, function(channel) {
        ...
        createCannel(channel.objects.backend);
        // connect to a signal
        backend.updateWeather.connect(function(w_index,w_degeres,w_date,w_night,w_txt) {
//            sendLog("d1:"+data1+"data2:"+data2);
            updateVariable(w_index,w_degeres,w_date,w_night,w_txt);
...
function createCannel(val,_temp){
    backend=val;
    temp=_temp;
    // ⚙ initialize app
    init();
...
function sendLog(value)
{
    ...
    backend.logFromHtml(value, function(callback) {
        //processThisShit(callback);
//        result = callback; //
    });
}
function updateVariable(w_index,w_degeres,w_date,w_night,w_txt){
    temptxt.textContent=w_degeres+'\u00B0';
    datetxt.textContent=w_date;
    nightTheme=w_night;
    changeWeather(weather[w_index],w_txt);
}

// you can also read/write properties of QtObject from QML side
function getPropertyValue()
{
//    var weatherIndex = backend.weatherInex;
...
var weather = [
            { type: 'snow', name: 'برفی'},
            { type: 'wind', name: 'باد شدید'},
            { type: 'rain', name: 'بارانی'},
            { type: 'thunder', name: 'طوفانی'},
            { type: 'sun', name: 'آفتابی'},
            { type: 'cloudy', name: 'ابری'}
        ];
...
function makeRain()
{
    ...
    var line;
if(!nightTheme){
     line = this['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
                                                                                                           fill: 'none',
     ...
     line = this['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
                                                                                                           fill: 'none',
                                                                                                           stroke: currentWeather.type == 'thunder' ? '#dbdbdb' : '#e0e0e0',
                                                                                                           strokeWidth: lineWidth
                                                                                                       });
}
...
function makeSplash(x, type)
{
    ...
    var splash;
    if(!nightTheme){
        splash = outerSplashHolder.path(points.join(' ')).attr({
                                                                   fill: "none",
        ...
        splash = outerSplashHolder.path(points.join(' ')).attr({
                                                                   fill: "none",
                                                                   stroke: type == 'thunder' ? '#dbdbdb' : '#e0e0e0',
                                                                   strokeWidth: 1
                                                               });
    }
...
function makeSnow()
{
    ...
    var y = sizes.card.offset.top+5;
    var endY;

    if(scale > 0.8)
    {
        newSnow = mysnow.clone().appendTo(outerSnowHolder)
        .attr({
                  fill: 'white'
              })
        // outerSnowHolder.circle(0, 0, 5)
        // 	.attr({
        // 		fill: 'white'
        // 	})
        endY = sizes.card.height;
    ...
    snow.push(newSnow);
    TweenMax.fromTo(newSnow.node, 3+ (Math.random() * 10), {rotation: 0,x: x, y:y,scale:0.5},{rotation: Math.random()* 360,x:x+((Math.random() * 150)-75),y:y+endY,scale:scale,yoyo:true, onComplete: onSnowEnd, onCompleteParams: [newSnow], ease: Power0.easeInOut});
    //	TweenMax.fromTo(newSnow.node, 3+ (Math.random() * 10), {x: x, y:y},{y:y+endY, onComplete: onSnowEnd, onCompleteParams: [newSnow], ease: Power0.easeIn});
    // TweenMax.fromTo(newSnow.node, 3 + (Math.random() * 10), {x: x, y: y}, {y: y+endY, onComplete: onSnowEnd, onCompleteParams: [newSnow], ease: Power0.easeIn})
    ...
    //	// TweenLite.to(newSnow.node, 2, {rotation:360, transformOrigin:"left 50%",repeat: -1});
    //    TweenMax.fromTo(newSnow.node, 5+(Math.random()*5), { rotation: 0},{rotation:360, ease: Power1.easeInOut, transformOrigin:"50% 50%" });
...
function startLightningTimer()
{
    ...
    {
        lightningTimeout = setTimeout(lightning, Math.random()*8000);
    }
}
...
function changeWeather(weather,w_txt)
{
    if(weather.data) weather = weather.data;
    reset();

    currentWeather = weather;
    currentWeather.name=w_txt;
    if(nightTheme){
      switch(weather.type){
      case "sun":
          datetxt.style.color="white";
          temptxt.style.color="white";
          summaryTxt.style.color="white";
          break;
      default:
          datetxt.style.color="#383838";
          temptxt.style.color="#383838";
          summaryTxt.style.color="#383838";
      }
    ...
    if(nightTheme){
     container.addClass("night"+weather.type);
     myweather.addClass("nightweather");
    }else{
     container.addClass(weather.type);
        myweather.removeClass("nightweather");
    }
    ...
    {
    case 'wind':
        TweenMax.to(settings, 3, {windSpeed: 4, ease: Power2.easeInOut});
        break;
    case 'sun':
        TweenMax.to(settings, 3, {windSpeed: 30, ease: Power2.easeInOut});
        break;
    case 'snow':
        TweenMax.to(settings, 0.5, {windSpeed: 30, ease: Power2.easeOut});
        TweenMax.to(settings, 3, {windSpeed: 2, ease: Power2.easeInOut});
        break;
    default:
        TweenMax.to(settings, 3, {windSpeed: 2, ease: Power2.easeOut});
        break;
    }
    // TweenMax.set(sunburst.node,{transform:"matrix(1,0,0,1,213.5,104)"});
    // rainCount
    ...
    case 'rain':
        ...
        break;
    case 'thunder':
        TweenMax.to(settings, 3, {rainCount: 40, ease: Power2.easeInOut});
        break;
    default:
    ...
    // sun position
    if(nightTheme){
       TweenMax.set(sun.node,{fill:"#b5b5b5"});
    }else{
       TweenMax.set(sun.node,{fill:"#F7ED47"});
    }

//    TweenLite.to("#myElement", 1, {text:"Strawberry", color:"#f00", ease:Linear.easeNone, paused:true, reversed:true});
    switch(weather.type)
    {
    case 'sun':
        ...
        break;
    case 'cloudy':
        TweenMax.to(sun.node, 4, {x: sizes.card.width / 1.3, y: sizes.card.height / 5, ease: Power2.easeInOut});
        //			TweenMax.to(sunburst.node, 4, {scale: 1, opacity: 0.8, y: (sizes.card.height/2) + (sizes.card.offset.top), ease: Power2.easeInOut});
        break;
    default:
